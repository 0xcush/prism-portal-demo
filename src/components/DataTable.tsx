...
export default function DataTable<T extends Record<string, any>>({
  ...
  // Sync with topbar search
  useEffect(() => {
    function handleTopbarSearch(e: Event) {
          ...
          <>
                      ...
                      return (
                        <th
                          ...
                          className={`px-5 py-3 text-xs font-medium text-slate-400 uppercase tracking-wider whitespace-nowrap ${
                            ...
                            i === 0
                              ? 'sticky left-0 bg-slate-50/50 z-10 after:absolute after:right-0 after:top-0 after:bottom-0 after:w-px after:bg-slate-100'
                              : ''
                          } ${col.className || ''}`}
                        >
                      ...
                      {columns.map((col, colIdx) => (
                        <td
                          ...
                          className={`px-5 py-3.5 text-sm ${
                            ...
                            colIdx === 0
                              ? 'sticky left-0 bg-white z-10 after:absolute after:right-0 after:top-0 after:bottom-0 after:w-px after:bg-slate-100'
                              : ''
                          } ${col.className || ''}`}
                        >
                          {col.render(row)}
            ...
            <div className="md:hidden divide-y divide-slate-100">
              {processed.map((row, rowIdx) => (
                ...
                >
                  {columns.map((col, colIdx) => (
                    <div key={col.key} className={colIdx === 0 ? 'mb-2' : 'flex items-center justify-between py-1'}>
                      {colIdx === 0 ? (
                        <div className="font-medium text-slate-800">{col.render(row)}</div>
                      ) : (
                        <>
                          <span className="text-xs text-slate-400 uppercase tracking-wider">{col.header}</span>
                          <span className="text-sm text-slate-600">{col.render(row)}</span>
                        </>
                      )}
                    </div>
                  ))}
